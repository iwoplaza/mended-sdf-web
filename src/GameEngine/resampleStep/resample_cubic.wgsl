...
fn main(
  ...
  let coord_hg = uv * vec2f(canvas.size) - vec2f(0.5f, 0.5f);      // fetch offsets and weights from filter texture
  ...
  coord_source00 = coord_source00 - hg_y.y * canvas.e_y;      // fetch four linearly interpolated inputs
